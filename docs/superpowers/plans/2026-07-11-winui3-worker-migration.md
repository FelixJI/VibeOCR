...
**Python/UI 真源补充（v0.4.28，`56eae97`）：** `add_text_layer_batch` 已将写层和持久化结果解耦；页面处理结果与落盘结果必须分别表达，持久化失败通过 `ocr_write_error` 汇总去重并在本轮 OCR 结束后一次提示，不能把整批包装成 HTTP 500。删除文字层是逐页流式操作，UI 使用不确定进度动画，同时保留当前页/总页数状态和取消入口。页处理状态与文字层来源是两个正交维度：状态仍为 `none/processing/done/failed`，来源为 `none/native/ocr`；OCR 文字层显示深绿，原生 PDF 文字层显示浅绿，无文字层显示灰色，迁移时不得把原生层折叠成 OCR 完成态。

**现行按钮与语义（不得按旧 UI 推断）：** 选中页操作为“顺时针90°”“逆时针90°”，未选页时必须明确提示；全页操作拆为“全部顺时针90°”“全部逆时针90°”，无需二次确认。“自动摆正”继续使用 OCR 方向检测；“横放摆正”“纵放摆正”只按考虑 `page.rotation` 后的页面宽高比处理选中页，不调用 OCR，并跳过已经处于目标方向的页面。文字层入口为“添加文字层”“为无文字层页添加文字层”“删除文字层”“预览文字层”。
...
- 固化 `ocr_write_error` 的批后汇总提示、写层/持久化结果解耦、删除文字层逐页流式进度与取消。
- 固化 `state` 与 `layer_source` 的正交投影，以及 `none/native/ocr` 对应灰/浅绿/深绿；原生文字层不得写入 OCR sidecar 已完成页。
...
**UI/E2E:** 覆盖打开/分页/缩略图虚拟化、选中页和全页双向旋转、自动/横放/纵放摆正、删除文字层的流式取消、四态处理状态与三类文字层来源、逐批 OCR 落盘、写层成功但持久化失败的批后提示、崩溃后续传、取消、文字层编辑、保存/另存/导出、并发 mutation 拒绝和 session 失效。用真实多批 PDF 在批间终止 Worker，重启后必须只重做最后未落盘批；最终压缩后的体积与一次性聚合保存处于同一门限，100 次循环无句柄、进程、共享内存或 `.bak` 持续增长。
