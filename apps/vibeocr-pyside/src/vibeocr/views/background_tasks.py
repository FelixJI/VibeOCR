...
class DependencyUpdateCheckTask(QObject):
    """共享的依赖版本检查 single-flight。

    启动自动检查和设置页按钮共用同一个实例。检查进行中再次请求不会新建
    worker；首个请求来源负责结果展示，避免启动提示与手动提示重复弹窗。
    generation 与 closing 防止旧任务或窗口关闭后的迟到结果触碰 UI。
    """
    ...
    def request(self, source: str) -> bool:
        ...
        def operation() -> dict:
            ...
            # 开发态由 uv 管理环境，不读取版本清单、不启动 pip 元数据探测。
            if env_manager.get_environment_mode(self._project_root) != "portable":
                return {}
            return dict(env_manager.detect_dependency_updates(self._project_root))
