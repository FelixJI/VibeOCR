...
EXCLUSIVE_MUTEX_NAME = r"Local\VibeOCR.Frontend.Exclusive.v1"
...
class FrontendExclusiveLock:
    ...
    def try_acquire(self) -> bool:
        ...
        if handle == 0:
            # CreateMutex 失败（极少见，如内存不足）。不阻断启动——退化为
            # 允许运行，避免把用户锁死在无法启动的状态。
            logger.warning("[FrontendExclusiveLock] CreateMutex 失败，放行启动")
            self._acquired = True
            return True
