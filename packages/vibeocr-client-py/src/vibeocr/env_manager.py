...
def get_project_root() -> Path:
    ...
    源码开发态只在当前模块确实位于 client source root 时返回仓库根；普通
    ...
    while current.parent != current:
        if (
            ...
            and (current / "packages" / "vibeocr-client-py").exists()
            and (current / "apps" / "vibeocr-pyside").exists()
        ):
            client_source = current / "packages" / "vibeocr-client-py" / "src"
            try:
                module_path.relative_to(client_source)
            except ValueError:
                pass
            else:
                return current
