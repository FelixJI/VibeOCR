...
class BackendClient:
    ...
    async def decode_qrcode(
        ...
        try:
            result = await self.call(
                "qrcode.decode", {"image": ref.to_descriptor()}, timeout=timeout
            )
    ...
    async def export_ocr(
        ...
        *,
        ...
        return await self.call("ocr.export", payload, timeout=timeout)
    ...
    async def open_pdf(
        ...
        return await self.call("pdf.open", {"file_path": file_path}, timeout=timeout)

    async def close_pdf(
        self, session_id: str, *, timeout: float | None = None
    ) -> bool:
        result = await self.call(
            "pdf.close", {"session_id": session_id}, timeout=timeout
        )
    ...
    async def render_pdf_page(
        ...
        *,
        ...
        if dpi is not None:
            payload["dpi"] = dpi
        result = await self.call("pdf.render_page", payload, timeout=timeout)
    ...
    # -- typed pipeline cache helpers ---------------------------------

    async def pipeline_cache_status(
    ...
    async def release_pipeline_cache(
        ...
        return [str(name) for name in result["released"]]

    async def preload_pipeline_cache(
        ...
        return {str(name): bool(ok) for name, ok in result["results"].items()}

    async def warmup_pipeline_cache(
        self, pipelines: list[str], *, timeout: float | None = None
    ) -> dict[str, bool]:
        result = await self.call(
    ...
    async def settings_snapshot(
        ...
        return await self.call("settings.snapshot", {}, timeout=timeout)
    ...
    async def generate_qrcode(
        ...
        *,
        ...
        result = await self.call("qrcode.generate", payload, timeout=timeout)
        return await self._read_worker_payload(result["image"])
    ...
    async def generate_qrcode_svg(
        ...
        *,
        ...
        if options:
            payload.update(options)
        result = await self.call("qrcode.generate_svg", payload, timeout=timeout)
