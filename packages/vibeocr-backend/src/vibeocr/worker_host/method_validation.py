...
def _request_pipeline_cache_status(p: dict[str, Any]) -> None:
    _closed(p, label="pipeline_cache.status request")
...
def _request_pipeline_cache_release(p: dict[str, Any]) -> None:
    _closed(
        p,
        required={"heavy_only"},
        label="pipeline_cache.release request",
    )
    _boolean(p["heavy_only"], "heavy_only")


def _response_pipeline_cache_release(p: dict[str, Any]) -> None:
    _closed(
        p,
        required={"released"},
        label="pipeline_cache.release response",
    )
    if not isinstance(p["released"], list) or not all(
        isinstance(item, str) and item in _PIPELINES for item in p["released"]
    ):
        raise MethodPayloadError("released must be an array of strings")


def _request_pipeline_cache_load(p: dict[str, Any]) -> None:
    _closed(p, required={"pipelines"}, label="pipeline cache load request")
    if not isinstance(p["pipelines"], list) or not all(
        isinstance(item, str) and item in _PIPELINES for item in p["pipelines"]
    ):
        raise MethodPayloadError("pipelines must be an array of strings")


def _response_pipeline_cache_load(p: dict[str, Any]) -> None:
    _closed(p, required={"results"}, label="pipeline cache load response")
    results = _object(p["results"], "results")
    for name, succeeded in results.items():
        if name not in _PIPELINES:
            raise MethodPayloadError(f"unknown pipeline result: {name}")
        _boolean(succeeded, f"results.{name}")
...
_VALIDATORS: dict[str, tuple[Validator, Validator]] = {
    ...
    "pipeline_cache.status": (
        _request_pipeline_cache_status,
        _response_pipeline_cache_status,
    ),
    "pipeline_cache.set_ttl": (
        _request_pipeline_cache_set_ttl,
        _response_pipeline_cache_set_ttl,
    ),
    "pipeline_cache.release": (
        _request_pipeline_cache_release,
        _response_pipeline_cache_release,
    ),
    "pipeline_cache.preload": (
        _request_pipeline_cache_load,
        _response_pipeline_cache_load,
    ),
