...
class OcrServiceAdapter:
    ...
    def preload_pipelines(self, pipelines: list[str]) -> dict[str, bool]:
        return dict(self._get_service().preload_pipelines(pipelines))

    def warmup_pipelines(self, pipelines: list[str]) -> dict[str, bool]:
        return dict(self._get_service().warmup_pipelines(pipelines))
...
class WorkerServiceComposition:
    ...
    def handlers(self, store: SharedPayloadStore) -> dict[str, Handler]:
        return {
            ...
            "pipeline_cache.status": PipelineCacheStatusHandler(
                boundary=self._ocr_adapter
            ).handle,
            "pipeline_cache.set_ttl": SetPipelineCacheTtlHandler(
                boundary=self._ocr_adapter
            ).handle,
            "pipeline_cache.release": ReleasePipelineCacheHandler(
                boundary=self._ocr_adapter
            ).handle,
            "pipeline_cache.preload": PreloadPipelineCacheHandler(
                boundary=self._ocr_adapter
            ).handle,
            "pipeline_cache.warmup": WarmupPipelineCacheHandler(
                boundary=self._ocr_adapter
            ).handle,
