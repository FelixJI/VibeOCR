...
from vibeocr.logging_context import configure_worker_stderr_logging
...
def _emit_ready(pipe_name: str) -> None:
    """Emit the sole serving-mode stdout protocol record."""
    sys.stdout.write(
        json.dumps(
            {
                "event": "worker.ready",
...
async def _serve(args: argparse.Namespace) -> int:
    ...
    _emit_ready(server.endpoint.name)
    ...
    *,
    ...
    async def handshake(payload: dict[str, Any], cancel: Any) -> dict[str, Any]:
        ...
        return {
            "worker_version": VIBEOCR_VERSION,
            "protocol_version": PROTOCOL_VERSION,
            "capabilities": ["ocr", "pdf", "qrcode", "pipeline_cache", "settings"],
            "python_version": ".".join(map(str, sys.version_info[:3])),
            "backend": backend,
