...
MSG_CACHE_STATUS = MessageType.CACHE_STATUS
...
class OCRWorkerProcess:
    ...
    def __init__(
        ...
        # 用 Lock（非 RLock）：recognize/preload/cache_status 互不嵌套调用，
        # 不需要可重入；普通 Lock 支持 acquire(timeout=)，让 execute_control
        # 能在锁被卡住的 RPC 占着时超时放弃（保护后续 OCR 不被无限阻塞）。
    ...
    def cache_status(
        ...
        try:
            protocol.write_message(
                MSG_CACHE_STATUS, b"", timeout=timeout, sender="main"
            )
            protocol.wait_for_read(timeout=timeout)
            msg_type, data = protocol.read_message(
                timeout=timeout, expected_sender="worker"
            ...
            if msg_type != MSG_ACK:
                raise OCRWorkerProcessError(
                    f"Worker {self.worker_id} 缓存状态未收到 ACK: {msg_type}"
                )
            result = json.loads(data.decode("utf-8")) if data else {}
            if not isinstance(result, dict):
                raise OCRWorkerProcessError("缓存状态响应必须是对象")
            return result
        except SharedMemoryProtocolError as exc:
            raise OCRWorkerProcessError(f"查询缓存状态失败: {exc}") from exc
