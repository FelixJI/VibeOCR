...
用法（CLI）::
    ...
    python -m vibeocr.build_manifest verify <archive.zip>
...
def create_manifest(
    ...
    Returns:
        可序列化为 JSON 的 manifest dict::
            ...
            {
              "version": 1,
              "created_by": "vibeocr.build_manifest",
              "entry_count": N,
              "total_bytes": M,
              "entries": [
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args:
        print("usage: python -m vibeocr.build_manifest verify <archive.zip>", file=sys.stderr)
        return 2
