...
class WorkerInfo:
    ...
    def is_available(self) -> bool:
        """检查 Worker 是否可用"""
        return self.state in (WorkerState.IDLE, WorkerState.BUSY) and self.process.is_ready
...
class WorkerManager:
    ...
    def __init__(
        ...
        logger.info(f"WorkerManager 初始化: max_workers={max_workers}, use_gpu={use_gpu}")
            ...
            for i in range(self.max_workers):
                ...
                if not info.process.is_running:
                    try:
                        ...
                        info.process.start(timeout=self.start_timeout)
    ...
    def _get_available_worker(self, wait_timeout: float = 5.0) -> Optional[WorkerInfo]:
        """获取可用的 Worker（轮询）
        ...
        while time.time() - start_time < wait_timeout:
            with self._workers_lock:
                ...
                for _ in range(len(self._workers)):
                    ...
                    self._round_robin_index = (self._round_robin_index + 1) % len(self._workers)
    ...
    def _recover_workers(self) -> None:
        ...
        with self._workers_lock:
            for worker_info in self._workers:
                if worker_info.state == WorkerState.ERROR or not worker_info.process.is_running:
            ...
            for worker_info in self._workers:
                ...
                if worker_info.state == WorkerState.BUSY:
                    ...
                    if idle_time > 300:  # 5 分钟
                        logger.warning(f"Worker {worker_info.worker_id} 可能卡死（无响应 {idle_time:.0f} 秒）")
                        # 强制重启
                        self._restart_worker(worker_info)
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "availability": stats["ready_workers"] / stats["max_workers"] if stats["max_workers"] > 0 else 0
        }
