...
_WARMUP_IMAGE_DATA: Optional[bytes] = None
...
def warmup_with_test_image(ocr_service, pipeline: Optional[str] = None) -> bool:
    """使用测试图片预热 OCR 服务
    ...
    try:
        ...
        # 执行虚拟识别
        result = ocr_service.recognize(test_image, options)
...
def warmup_worker_process(worker_process, timeout: float = 60.0) -> bool:
    ...
    try:
        ...
        # 执行识别（这会触发模型加载）
        result = worker_process.recognize(test_image, options_dict, timeout=timeout)
        
        elapsed = time.time() - start_time
        _logger.info(f"Worker {worker_process.worker_id} 预热完成，耗时: {elapsed:.2f}秒")
        return True
